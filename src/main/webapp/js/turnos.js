...
function abrirEditar(turno) {
  document.getElementById("modalTitle").innerHTML =
    '<i class="fas fa-edit"></i> Modificar Turno Agendado';
...
function onPacienteChange() {
  ...
  document.getElementById("obra-social-display").value =
    paciente.obraSocialNombre || "Sin cobertura";
...
function cancelarTurno(id) {
  confirmarEliminacion("¿Confirma la cancelación de este turno?", function () {
...
function filtrosAplicados() {
  ...
  return (
    url.searchParams.get("idMedico") ||
    url.searchParams.get("desde") ||
    url.searchParams.get("hasta") ||
    url.searchParams.getAll("estado").length > 0
  );
}

function limpiarParametrosFiltro() {
  const url = new URL(window.location);
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Mostrar botón solamente si hay filtros
  if (filtrosAplicados() && btnLimpiar) {
  ...
  if (btnLimpiar) {
    btnLimpiar.addEventListener("click", () => {
      limpiarParametrosFiltro();
      window.location.href = "/app/turnos"; // recargar sin filtros
    });
  }

  // Cada vez que se aplica el filtro, limpiar la URL visual
